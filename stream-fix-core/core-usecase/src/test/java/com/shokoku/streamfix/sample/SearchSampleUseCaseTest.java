...
class SearchSampleUseCaseTest {
    ...
    class SearchSampleUseCaseBusinessScenarioTest {
        ...
        void shouldHandleMultiLanguageSampleNames() {
            ...
            // when & then
            assertThat(createTestImplementation(koreanSample).getSample().name())
                .isEqualTo(koreanSample);
            ...
            assertThat(createTestImplementation(emojiSample).getSample().name())
                .isEqualTo(emojiSample);
        }
        ...
        void shouldHandleLongSampleNames() {
            // given - 매우 긴 샘플명
            String longSampleName = "이것은 매우 긴 샘플명입니다. ".repeat(10) +
                                   "실제 비즈니스에서는 이런 긴 이름도 처리할 수 있어야 합니다.";
