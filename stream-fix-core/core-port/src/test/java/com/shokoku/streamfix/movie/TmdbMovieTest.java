...
class TmdbMovieTest {
  ...
  class CreateTmdbMovieTest {
    ...
    void shouldCreateAdultTmdbMovie() {
      // given & when
      TmdbMovie adultMovie = new TmdbMovie("성인 영화", true, List.of("드라마"), "성인 대상 영화", "2024-01-01");
    ...
    void shouldCreateTmdbMovieWithEmptyGenres() {
      // given & when
      TmdbMovie movie =
          new TmdbMovie("장르 미정", false, Collections.emptyList(), "장르가 정해지지 않은 영화", "2024-12-25");
  ...
  class TmdbMovieFieldValidationTest {
    ...
    @ValueSource(strings = {"기생충", "미나리", "올드보이", "타짜: 원 아이드 잭", "신과함께-죄와 벌"})
    @DisplayName("다양한 한국어 영화 제목으로 객체를 생성할 수 있다")
    void shouldCreateTmdbMovieWithKoreanTitles(String koreanTitle) {
      // when
      TmdbMovie movie = new TmdbMovie(koreanTitle, false, List.of("드라마"), "한국 영화", "2020-01-01");
    ...
    void shouldCreateTmdbMovieWithVariousGenres(List<String> genres, int expectedSize) {
      // when
      TmdbMovie movie = new TmdbMovie("테스트 영화", false, genres, "테스트용 영화", "2024-01-01");
    ...
    void shouldCreateTmdbMovieWithLongOverview() {
      // given
      String longOverview =
          "이것은 매우 긴 영화 개요입니다. ".repeat(10) + "영화의 스토리, 캐릭터, 배경 등을 상세히 설명하는 내용이 포함되어 있습니다.";

      // when
      TmdbMovie movie =
          new TmdbMovie("서사 대작", false, List.of("드라마", "서사"), longOverview, "2024-06-01");
  ...
  class TmdbMovieReleaseDateTest {
    ...
    @ValueSource(strings = {"2024-01-01", "2023-12-31", "2025-06-15", "1999-01-01", "2030-12-25"})
    @DisplayName("유효한 날짜 형식으로 영화를 생성할 수 있다")
    void shouldCreateTmdbMovieWithValidDateFormat(String releaseDate) {
      // when
      TmdbMovie movie = new TmdbMovie("날짜 테스트 영화", false, List.of("테스트"), "날짜 형식 테스트", releaseDate);
    ...
    void shouldIdentifyUpcomingMovie() {
      ...
      TmdbMovie futureMovie =
          new TmdbMovie("미래 영화", false, List.of("SF"), "미래에 개봉할 영화", futureDate);
    ...
    void shouldIdentifyReleasedMovie() {
      ...
      TmdbMovie pastMovie = new TmdbMovie("과거 영화", false, List.of("드라마"), "이미 개봉한 영화", pastDate);
  ...
  class TmdbMovieBusinessLogicTest {
    ...
    void shouldCheckIfMovieContainsGenre() {
      // given
      TmdbMovie actionMovie =
          new TmdbMovie(
              "액션 영화", false, Arrays.asList("액션", "스릴러"), "액션과 스릴러가 포함된 영화", "2024-01-01");
    ...
    void shouldGetMovieTitleLength() {
      // given
      TmdbMovie movie =
          new TmdbMovie("매우 긴 제목을 가진 영화", false, List.of("드라마"), "제목이 긴 영화", "2024-01-01");
    ...
    void shouldCheckAdultContent() {
      // given
      TmdbMovie adultMovie = new TmdbMovie("성인 영화", true, List.of("성인"), "성인 대상", "2024-01-01");
      TmdbMovie familyMovie = new TmdbMovie("가족 영화", false, List.of("가족"), "전체 관람가", "2024-01-01");
  ...
  class TmdbMovieEqualityTest {
    ...
    void shouldBeEqualWhenSameContent() {
      ...
      TmdbMovie movie1 = new TmdbMovie("동일 영화", false, genres, "같은 영화", "2024-01-01");
      TmdbMovie movie2 = new TmdbMovie("동일 영화", false, genres, "같은 영화", "2024-01-01");
    ...
    void shouldNotBeEqualWhenDifferentContent() {
      // given
      TmdbMovie movie1 = new TmdbMovie("영화1", false, List.of("액션"), "첫 번째 영화", "2024-01-01");
      TmdbMovie movie2 = new TmdbMovie("영화2", false, List.of("드라마"), "두 번째 영화", "2024-01-02");
