...
class TmdbPageableMoviesTest {
  ...
  class PageableMoviesPageInfoTest {
    ...
    void shouldCreateTmdbPageableMoviesWithVariousMovieListSizes(
        List<TmdbMovie> movies, int expectedSize) {
      // when
  ...
  class PageableMoviesBusinessLogicTest {
    ...
    void shouldSummarizePageInfo() {
      ...
      // then
      assertThat(summary).contains("페이지: 3").contains("영화 수: 3").contains("다음 페이지: 있음");
    }
    ...
    private long countAdultMovies(TmdbPageableMovies pageableMovies) {
      return pageableMovies.tmdbMovies().stream().filter(TmdbMovie::isAdult).count();
    }

    private long countFamilyMovies(TmdbPageableMovies pageableMovies) {
      return pageableMovies.tmdbMovies().stream().filter(movie -> !movie.isAdult()).count();
    }
